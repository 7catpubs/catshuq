...
[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/7catpubs/CfDaoHang)
...
## 重点说明
如果通过本项目引导的方式进行CF部署，那么，完成CF上面的Workers 和 Pages上面的项目设置当中的“变量和机密”并设置好网站参数之后，就可以在项目设置的“构建”当中断开本项目的链接了，这样，后续本项目有文件改变也不需要重新部署了。
...
![演示图](https://private-user-images.githubusercontent.com/14195003/512047595-06e62178-6add-4694-8a22-dc369b044c91.png?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..xvATzLeBUDI0K1e-fhYGUxWwcz0QN1oU1cYBhn3B_ZA)
## 🌐 在线演示
...
-   👥 **游客模式** - 访客无需登录即可浏览所有导航内容
-   🔐 **管理员认证** - 管理员登录后可编辑和管理导航数据
...
**Q: 游客模式和管理员模式有什么区别？**  
A: 游客模式只能查看导航内容，无法编辑；管理员模式可以添加、修改、删除导航内容。所有访客默认进入游客模式，需要登录才能进入管理员模式。

**Q: 如何让导航站完全公开访问？**  
A: 导航站默认就是公开的，访客无需登录即可浏览。如果您希望管理员功能也不显示登录按钮，可以将环境变量`AUTH_ENABLED`设置为`false`。

**Q: 我想关闭游客访问，只允许登录用户查看，可以吗？**  
A: 当前版本不支持此功能。如果您有此需求，可以考虑在 Cloudflare Workers 层面添加额外的访问控制。
...
## 📅 更新日志
### v1.1.0(2025-11-07) - 游客模式与权限优化
- 🎉 **重大更新**：新增游客模式与管理员模式双模式设计
- 👥 **游客访问**：访客无需登录即可浏览所有导航内容
- 🔐 **权限控制**：读操作无需认证，写操作需要管理员权限
- 🎯 **管理入口**：增加"管理员登录"按钮，登录后进入管理模式
- 🔒 **安全优化**：后端API区分读写操作，只对写操作进行认证验证
- 🎨 **界面优化**：游客模式下自动隐藏所有编辑功能按钮
- 💡 **用户体验**：支持在游客模式和管理员模式之间无缝切换
- 📝 **登录方式**：改为弹窗式登录对话框，不再阻断页面访问

**升级说明**：
- 现在 `AUTH_ENABLED=false` 时，前端页面将直接以游客模式展示，无需登录
- `AUTH_ENABLED=true` 时，前端页面同样以游客模式展示，点击"管理员登录"按钮后可进入管理模式
- 所有读取操作（GET请求）无需认证，游客可直接访问
- 所有写入操作（POST/PUT/DELETE请求）需要管理员认证
